 ...
 * - Production: Logs blocked in browser devtools, but available in Cloudflare Pages logs
 */
...
export const logger = {
  // Always log errors (visible in Cloudflare Pages logs, blocked in production browser devtools)
  error: (...args: unknown[]) => {
  ...
  // Log in development and Cloudflare Pages functions (blocked in production browser devtools)
  log: (...args: unknown[]) => {
    if (!isProduction || !isBrowser) {
      console.log(...args);
    }
